...
public class ClientPanel extends JPanel {
    ...
    private void initialiserComposants() {
        ...
        tableauClients.setDefaultEditor(Object.class, null); // Désactive l'édition des cellules du tableau
            ...
            public void mouseClicked(java.awt.event.MouseEvent evt) { // Méthode appelée quand on clique sur le tableau
                if (evt.getClickCount() == 2) { // Vérifie si c'est un double-clic
                    ...
                    if (row >= 0) { // Vérifie si une ligne est sélectionnée
                        ...
                        String nomPrenom = tableauClients.getValueAt(row, 1) != null ? tableauClients.getValueAt(row, 1).toString() : "";
                        String matriculeFiscale = tableauClients.getValueAt(row, 2) != null ? tableauClients.getValueAt(row, 2).toString() : "";
                        String adresse = tableauClients.getValueAt(row, 3) != null ? tableauClients.getValueAt(row, 3).toString() : "";
                        String numTelephone = tableauClients.getValueAt(row, 4) != null ? tableauClients.getValueAt(row, 4).toString() : "";
                        // Remplit les champs avec les données
                        txtNomPrenom.setText(nomPrenom);
                        txtMatriculeFiscale.setText(matriculeFiscale);
                        txtAdresse.setText(adresse);
                        txtNumTelephone.setText(numTelephone);
                    }
                }
            }
        });
    ...
    private void btnAjouterClient_Click() {
        ...
        // Vérifie si le matricule fiscale est vide
        if (matriculeFiscale.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Veuillez remplir le matricule fiscale !", "Erreur", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // Vérifie si l'adresse est vide
        if (adresse.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Veuillez remplir l'adresse !", "Erreur", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try {
            ...
            if (numTelephoneText.length() != 8) { // Vérifie si le numéro est de 8 chiffres
                JOptionPane.showMessageDialog(this, "Le numéro de téléphone doit contenir exactement 8 chiffres !", "Erreur", JOptionPane.ERROR_MESSAGE); // Affiche un message d'erreur
                return; // Arrête la méthode
            }
    ...
    private void btnSupprimerClient_Click() {
        ...
        int choix = JOptionPane.showConfirmDialog(this, "Êtes-vous sûr de vouloir supprimer ce client ?", "Confirmation", JOptionPane.YES_NO_OPTION); // Affiche une fenêtre de confirmation
